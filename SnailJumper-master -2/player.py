...
def create_input(obstacles, player_x, player_y):

    if len(obstacles) == 0:
        distance1 = random.randint(177, 430)
        distance2 = (-100) - player_y
        distance3 = random.randint(177, 430)
        distance4 = (-100) - player_y
    elif len(obstacles) == 1:
        distance1 = obstacles[0]['x'] - player_x
        distance2 = obstacles[0]['y'] - player_y
        distance3 = random.randint(177, 430)
        distance4 = (-100) - player_y
    # elif len(obstacles) == 2:
    #     distance1 = obstacles[0]['x'] - player_x
    #     distance2 = obstacles[0]['y'] - player_y
    #     distance3 = obstacles[1]['x'] - player_x
    #     distance4 = obstacles[1]['y'] - player_y
    else:
        distance1 = obstacles[0]['x'] - player_x
        distance2 = obstacles[0]['y'] - player_y
        distance3 = obstacles[1]['x'] - player_x
    ...
    x = np.array(([distance1], [distance2], [distance3], [distance4]), dtype=np.float64)
    normalized(x)
...
class Player(pygame.sprite.Sprite):
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        x = create_input(obstacles, player_x, player_y)
        res = self.nn.forward(x)
